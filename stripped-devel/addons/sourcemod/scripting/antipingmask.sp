...
public OnPluginStart( )
{
	...
	/* Debug command to see cl_cmdrate for all players */
	RegConsoleCmd("sm_apm_debug", cmd_apmDebug, "Displays player cl_cmdrate debug information");
	
	/* Create handler CVARs. */
	g_Cvar_Handler 			= CreateConVar( "apm_handler", "1", "Determines how Anti-Ping Mask should handle ping masked clients.", FCVAR_NONE | FCVAR_NOTIFY | FCVAR_DONTRECORD, true, 0.0, true, 3.0 );
	g_Cvar_BanLength 		= CreateConVar( "apm_banlength", "10", "The length in minutes that a player is banned for ping masking.", FCVAR_NONE | FCVAR_DONTRECORD );
	g_Cvar_TimerInterval 		= CreateConVar( "apm_timerinterval", "8", "The interval time, in seconds, that the timer should tick at to check for ping masking clients.", FCVAR_NONE | FCVAR_DONTRECORD );
	g_Cvar_AdminImmunity		= CreateConVar( "apm_adminimmunity", "0", "Admins immune to ping mask log/annoy/kick/ban.", FCVAR_NONE | FCVAR_DONTRECORD, true, 0.0, true, 1.0);
	g_Cvar_LogMode			= CreateConVar( "apm_logmode", "0", "Enable/disable logging ping masking clients.", FCVAR_NONE | FCVAR_DONTRECORD, true, 0.0, true, 1.0 );
...
CheckPingMasked( client )
{
	...
	if (GetConVarBool( g_Cvar_AdminImmunity ))
	{
		if( !IsClientConnected( client ) || !IsClientInGame( client ) || IsFakeClient( client ) || CheckCommandAccess(client, "antipingmask_immunity", ADMFLAG_CHEATS) )
			return;
	} else {
	...
	if( ! IsValidCmdRate( cmdRate ) )
	{
		...
		//GetClientAuthID( client, AuthId_Engine, cl_SteamID, sizeof( cl_SteamID ), true );
		...
		{
			case 0:	// Do nothing but add to server log
			{
				// Fill server logs
				PrintToServer( "[Anti-Ping Mask] Detected ping masking (%s) with setting of: %s", cl_Name, cmdRate);
			}
			case 1: // Annoy the client(s)
			{
				// Fill server logs
				PrintToServer( "[Anti-Ping Mask] Detected ping masking (%s) with setting of: %s", cl_Name, cmdRate);

				// Chat spam the client
				PrintToChat( client,"%c[Anti-Ping Mask]%c Detected you are ping masking, open your console to see fix", COLOR_GREEN, COLOR_DEFAULT, cl_Name);

				// Annoying center screen msg
				PrintCenterText( client, "Detected you are ping masking, open your console to see fix");

				// Annoying hint box
				PrintHintText( client, "Detected you are ping masking, open your console to see fix");

				// Flood their console
				PrintToConsole( client, "[Anti-Ping Mask] ==========================================================");
				PrintToConsole( client, "[Anti-Ping Mask] Detected you are ping masking");
				PrintToConsole( client, "[Anti-Ping Mask] Your current cl_cmdrate is %s", cmdRate);
				PrintToConsole( client, "[Anti-Ping Mask] Please set cl_cmdrate to a number only and higher than 30");
				PrintToConsole( client, "[Anti-Ping Mask] To get back to valves default enter the following command");
				PrintToConsole( client, "[Anti-Ping Mask] cl_cmdrate 30");
				PrintToConsole( client, "[Anti-Ping Mask] =========================================================");
			}
			case 2: // Kick Client
			{
				...
				/* Log to server */
				PrintToServer( "[Anti-Ping Mask] Detected ping masking (%s) with setting of: %s", cl_Name, cmdRate);
				PrintToServer( "[Anti-Ping Mask] Kicked %s", cl_Name);
			
				/* Announce that player was kicked for ping masking. */
				PrintToChatAll( "%c[Anti-Ping Mask]%c %s %t", COLOR_GREEN, COLOR_DEFAULT, cl_Name, "Player kicked for ping masking" );
			}
			...
			case 3: // Ban Client
			{
				...
				Format( cl_BanMessage, sizeof( cl_BanMessage ), "%t", "Banned for ping masking" );
				...
				PrintToChatAll( "%c[Anti-Ping Mask]%c %s %t", COLOR_GREEN, COLOR_DEFAULT, cl_Name, "Player banned for ping masking" );
			}
		}
...
/**
* Anti-Ping Mask Debug
*/
DebugPingMasked( client, cmdclient )
{
	if( !IsClientConnected( client ) || !IsClientInGame( client ) || IsFakeClient( client ))
		return;
	...
	if( IsValidCmdRate( cmdRate ) )
	{
		PrintToConsole(cmdclient, "[Anti-Ping Mask] [ ] CL_CMDRATE: %s Player: %s", cmdRate, cl_Name);
	} else {
		PrintToConsole(cmdclient, "[Anti-Ping Mask] [!] CL_CMDRATE: %s Player: %s", cmdRate, cl_Name);
	}	
	//PrintToConsole(cmdclient, "Player: %-32N CL_CMDRATE: %-32N", client, cmdRate);
}

public Action:cmd_apmDebug(client, args)
{
	PrintToConsole(client, "[Anti-Ping Mask] cl_cmdrate Debug");

	/* Loop the client connections and check for masks. */
	new _maxClients = GetMaxClients( );
	for( new i = 1; i < _maxClients; i++ ) {
		DebugPingMasked( i, client );
	}
	PrintToConsole(client, "[Anti-Ping Mask] ================");
