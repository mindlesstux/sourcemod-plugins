...
#define PREFIX "\x04NoWall Zones \x01> \x03"
...
{
	new Handle:hVersion = CreateConVar("sm_nowallzones_version", PLUGIN_VERSION, "Anti Rush Version", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_DONTRECORD);
	if(hVersion != INVALID_HANDLE)
		SetConVarString(hVersion, PLUGIN_VERSION);
	
	g_hZones = CreateArray();
	
	HookEvent("arrow_impact", Event_OnBulletImpact);
	HookEvent("teamplay_round_start", Event_OnRoundStart);
	
	RegAdminCmd("sm_nowall", Command_SetupZones, ADMFLAG_CONFIG, "Sets up invisable wall zones");
}
	...
	if(buttons & IN_USE)
	{
		if(!g_bPlayerPressesUse[client] && g_iPlayerCreatesZone[client] != NO_POINT)
		{
			...
			else if(g_iPlayerCreatesZone[client] == SECOND_POINT)
			{
				...
				TE_SendBeamBoxToClient(client, g_fTempZoneVector1[client], g_fTempZoneVector2[client], g_iLaserMaterial, g_iHaloMaterial, 0, 30, 5.0, 5.0, 5.0, 2, 1.0, {0,0,255,255}, 0);
				
				// Confirm the new zone
				new Handle:menu = CreateMenu(MenuHandler_SaveNewZone);
				SetMenuTitle(menu, "Add this new nowall zone?");
...
public Action:Event_OnRoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for(new i=MaxClients;i<iEnts;i++)
	{
		...
		&& StrContains(sClassName, "sm_nowall") != -1)
		{
			AcceptEntityInput(i, "Kill");
		}
	}
	
	// Create the func_wall
...
public Action:Command_SetupZones(client, args)
{
	if(!client)
	{
		ReplyToCommand(client, "NoWall Zones > Command is in-game only.");
	...
	SetMenuTitle(menu, "NoWall Zones on %s:", sMap);
...
public MenuHandler_SelectVector(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		if(StrEqual(info, "show") || StrEqual(info, "teleport"))
		{
			...
			TE_SendBeamBoxToClient(param1, fVec1, fVec2, g_iLaserMaterial, g_iHaloMaterial, 0, 30, 5.0, 5.0, 5.0, 2, 1.0, {0,0,255,255}, 0);
...
public MenuHandler_SaveNewZone(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		else if(StrEqual(info, "save"))
		{
			...
			// Spawn the func_wall
			SpawnFuncNoWall(g_iPlayerEditsZone[param1]);
			
			PrintToChat(param1, "%sSaved new NoWall zone", PREFIX);
...
public PanelHandler_ConfirmDelete(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		if(param2 == 1)
		{
			...
			KillNoWallEntity(g_iPlayerEditsZone[param1]);
			...
			if(g_iPlayerEditsZone[param1] < iSize)
			{
				...
				for(new i=MaxClients+1;i<iEnts;i++)
				{
					...
					&& IsValidEdict(i)
					&& GetEdictClassname(i, sClassName, sizeof(sClassName))
					&& StrContains(sClassName, "func_wall") != -1
					&& GetEntPropString(i, Prop_Data, "m_iName", sClassName, sizeof(sClassName))
					&& StrContains(sClassName, "sm_nowall_") != -1)
					{
						iIndex = StringToInt(sClassName[11]);
						if(iIndex > g_iPlayerEditsZone[param1])
						{
							Format(sClassName, sizeof(sClassName), "sm_nowall_%d", --iIndex);
...
ParseZoneConfig()
{
	...
	GetCurrentMap(sMap, sizeof(sMap));
	BuildPath(Path_SM, sConfigFile, sizeof(sConfigFile), "configs/nowallzones/%s.cfg", sMap);
	...
	new Handle:kv = CreateKeyValues("NoWallZones");
...
SaveZonesToConfig()
{
	// Delete from config file
	decl String:sConfigFile[PLATFORM_MAX_PATH], String:sMap[64];
	BuildPath(Path_SM, sConfigFile, sizeof(sConfigFile), "configs/nowallzones");
...
SpawnFuncNoWall(iZoneIndex)
{
	...
	new iEnt = CreateEntityByName("func_wall");
	
	decl String:sZoneName[32];
	DispatchKeyValue(iEnt, "spawnflags", "64");
	Format(sZoneName, sizeof(sZoneName), "sm_nowall_%d", iZoneIndex);
