...
new const String:PLUGIN_VERSION[] = "1.1.7f";
...
public OnPluginStart()
{
	...
	/* Board Settings */
	g_Cvar_ForumSoftwareID = CreateConVar("rtf_forum_software_id", "", "Which board you use? Visit https://bitbucket.org/norcalbots/report-to-forums/wiki/Supported%20Forums", _, true, 1.0, true, 10.0);
...
public OnConfigsExecuted()
{
	...
	LogMessage("Forum Software ID: %d", g_iForumSoftwareID);
...
public CreateReportThread(iClient)
{
	...
	switch(g_iForumSoftwareID)
	{
		...
		{
			GetWebSafeString(g_szThreadTitle[iClient], szSafeTitle, sizeof(szSafeTitle), false);
			Format(szSQLQuery, sizeof(szSQLQuery), "INSERT INTO %sthread (node_id, title, reply_count, view_count, user_id, username, post_date, discussion_state, discussion_open) VALUES ('%d', '%s', '0', '1', '%d', '%s', '%d', 'visible', '1');", g_szTablePrefix, g_iForumID, szSafeTitle, g_iUserID, g_szUserName, g_iTimeStamp[iClient]);
		}
	}
...
public GetPostId(iClient)
{
	...
	switch(g_iForumSoftwareID)
	{
		...
		case FORUM_XENFORO:
		{
			Format(szSQLQuery, sizeof(szSQLQuery), "SELECT post_id FROM %spost WHERE post_date='%d';", g_szTablePrefix, g_iTimeStamp[iClient]);
		}
	}
...
public SetPostId(iClient)
{
	...
	switch(g_iForumSoftwareID)
	{
		...
		case FORUM_XENFORO:
		{
			Format(szSQLQuery, sizeof(szSQLQuery), "UPDATE %sthread SET first_post_id='%d', last_post_date='%d', last_post_id='%s', last_post_user_id='%d', last_post_username='%s' WHERE thread_id='%d';", g_szTablePrefix, g_iPostID[iClient], g_iTimeStamp[iClient], g_iPostID[iClient], g_iUserID, g_szUserName, g_iThreadID[iClient]);
		}
	}
...
public GetUserPostInfo(iClient)
{
	...
	switch(g_iForumSoftwareID)
	{
		...
		case FORUM_XENFORO:
		{
			Format(szSQLQuery, sizeof(szSQLQuery), "SELECT message_count FROM %suser WHERE user_id='%d';", g_szTablePrefix, g_iUserID);
		}
	}
...
public UpdateUserPostCount(iClient, iPostCount)
{
	...
	switch(g_iForumSoftwareID)
	{
		...
		case FORUM_XENFORO:
		{
			Format(szSQLQuery, sizeof(szSQLQuery), "UPDATE %suser SET message_count='%d', last_activity='%d' WHERE user_id='%d';", g_szTablePrefix, iPostCount, g_iTimeStamp[iClient], g_iUserID);
		}
	}
